...
BROKER_URL = (
    ...
    + "everyone"
    + "@"
    + geode_config.wis2.broker_address
    + ":"
    + str(geode_config.wis2.broker_port)
    + "/mqtt"
)
...
def test_wis2_watcher():
    ...
    geode_config.wis2.download_dir_root = str(tmp_path)

    client = MQTTPubSubClient(BROKER_URL, options={"verify_certs": True})

    client.bind("on_message", watcher.on_message)

    print(f"[*] Connecting to {geode_config.wis2.broker_address}:{geode_config.wis2.broker_port} via WSS...")
